...
|---|---|---|---|---|
| **P1** | ~~Viz 2 = single point-in-time snapshot~~ **RESOLVED (superseded 2026-06-29 → -07-08).** Viz 2 is now a **real multi-day GPU_UTIL time-series** from the corrected per-cluster query (`avg( avg by (UUID)( GPU_UTIL{modelName=...} ) )`, 24×7, 5-min step, card-weighted 216:20). As of **2026-07-08 the ETL accumulates across pulls** (`fact_workload_util.py` globs every dated `_b200*`/`_h100*` pair, dedupes overlaps by freshest-file mtime) — so the series grows week-on-week like the token facts, no longer bounded by a single ~10-day retention pull. Current real coverage Jun 19–26 (Jun 15–18 was a one-off hand-backfill, **not** ETL output — the only remaining non-real rows). | *(was: no reliable workload time-series)* | **drop the Jun 15–18 hand-backfilled rows** once ≥2 accumulated real pulls exist; otherwise done | fully real accumulated `fact_workload_util` trend, all rows from Grafana |
...
| **P4** | **✅ Largely resolved (2026-06-29):** gptoss cluster attribution corrected after inspecting K8s config. Production `gptoss-120b` → **H100 only** (Prda, via `llm-platform-helm-router-service`). B200 gptoss instances serve under **`gptoss-120b-benchmark`** (+ `-test`, `-guarded`) — all pointing to `inference-spd` (B200). `gptoss-120b-benchmark` added to `dim_models` whitelist (real traffic confirmed: Digital Product Squad B 36M tokens, SPF Product Squad A 5.9M tokens, ~98% duty). Card snapshot re-attributed: 12 B200 cards → `gptoss-120b-benchmark@B200`. Fleet duty updated from 55% → 64%. `gptoss-120b-guarded` registered (0 traffic in Jun window, future-ready). **Remaining:** fallback chain means `-benchmark` traffic can fall back to H100 `gptoss-120b` if Superpod is down — minor, acceptable. <br>**⚠ Update (2026-07-02) — the B200 re-attribution was a HIDDEN MANUAL STEP, now automated.** The card-snapshot builder cleaned both clusters' pods to `gptoss-120b`, so a plain re-run emitted `gptoss-120b@B200` (not in the B200 whitelist → 12 cards orphaned → loaded silently dropped 127→115). Someone had been hand-renaming those 12 rows after each build. **Fixed durably:** `build_fact_card_snapshot.py` now disambiguates by cluster (`gptoss-120b` core on B200 → `gptoss-120b-benchmark`), so `python3 build_fact_card_snapshot.py` produces a clean file with no manual edit. **Caveat:** the heuristic assumes B200 gptoss = benchmark; it can't tell benchmark from `-guarded`/`-test` by pod name — revisit if `-guarded` ever gets its own B200 cards. | K8s inspection (2026-06-29) confirmed the split. `litellm_deployment_total_requests_total` `api_base` analysis was the initial lead. | No further action needed unless gptoss-120b-guarded accumulates real traffic (re-pull and it auto-appears) | Both clusters tracked with correct model names |
| **P15** | **Card-snapshot ↔ dim_models name discipline** — a stale published `data_real/fact_card_snapshot.csv` named xiaomi `xiaomi-mimo@B200` while `dim_models`/token facts use `xiaomi-mimo-v2-5@B200`. The 4 loaded cards silently dropped from `MODELS` (donut/KPI/duty read **123/236 = 52.1%** instead of **127/236 = 53.8%**) while the card map — which reads raw `CARD_ROWS` — still counted them, so the two panels disagreed with no error. `pod_model_map.py` was already correct (`xiaomi-mimo`→`xiaomi-mimo-v2-5`); the published CSV was just built before that map entry. **Fixed:** regenerated the snapshot. **Guardrail added (permanent, see below):** `rollup.js` now `console.warn`s on any card `model_uid` absent from `dim_models`. (Found 2026-07-01 via a live `?data=real` audit.) | snapshot built from a `pod_model_map` older than a name change → silent join drop | **automated ETL (P11)** rebuilds the snapshot on every pull from the current map → no stale artifact | contract-level check in the pipeline; guardrail stays as a runtime backstop |
...
| **P16** | **Model Throughput band is B200-only** — the H100 cluster is not covered at all. | no serving-benchmark curves exist for **H100** | **H100 curves exported** (same NVIDIA tool, ISL/OSL 8K/1K) | band can cover both clusters |
| **P17** | **Band covers only the GPUs running BENCHMARKED models** — real view: **32 of the 60 loaded B200 GPUs (53%)**. The other 28 (4 deployed models) have **no benchmark data** and are **excluded from both numerator and denominator**, named in the caption. *This is deliberate, and replaces an earlier version that proxied them from similar models + a median estimate — rejected by management (2026-07-13) as too assumption-heavy.* Excluding is honest; zero-filling would understate the models we did measure. | only 5 of the 9 relevant models were benchmarked (and 1 of those 5 isn't deployed) | **benchmark the 4 un-benchmarked deployed models.** Same tool, 4 more exports. | band covers all 60 loaded B200 GPUs with **zero** proxies or estimates |
| **P18** | **Deployed precision is assumed = benchmarked precision.** Confirmed only for `glm5-1` (its pod name carries `-fp4`). **The only assumption left in the band — a wrong label is a 2–4× error on that model.** | pod names carry no precision suffix for the other models | **confirm each model's precision against the serving configs** (10-minute check) | precision is a verified field, not an assumption |
| **P21** | **SLA fixed at 60 tok/s/user — no selector.** 60 is the only target every benchmarked model can meet: one model at fp16 has a hard single-user ceiling near **80 tok/s/user**, so an 80 or 100 tok/s SLA would disqualify it outright (it would contribute 0). The SLA acts as a *filter*, not just a dial. | thin low-CU curve coverage + fp16 precision on that model | **fp4 curves for it** (raises its ceiling ~3×) **+ low-CU points** for all models | an SLA selector (60 / 80 / 100) with every model qualifying at each |
...
- **`rollup.js` orphan guardrail** — console-warns when a `fact_card_snapshot` `model_uid` isn't in `dim_models` (would otherwise drop silently from MODELS/donut/KPI while still counting in the card map). Permanent runtime backstop, not a patch (added 2026-07-02, §P15).
- **Per-fact refresh model — two philosophies (decided 2026-07-08, see `RAW_DATA/DATA_REFRESH.md`):** each weekly Grafana pull is saved as a **new dated raw file**, and the ETL scripts **glob + concat** — but the *retention* policy differs by fact and is deliberate, not a bug:
  - **Tokens (Viz 6/4) + Workload-util (Viz 2) = accumulate forever.** These are **trends**; every pull is kept, none pruned. (`fact_model_token_weekly.py`, `fact_token_usage_monthly.py`, and — as of 2026-07-08 — `fact_workload_util.py`, which was changed from a fixed-file overwrite to a glob so Viz 2 grows too. Overlapping dates dedupe by freshest-file mtime.)
  - **Duty (Viz 1 / Time-in-Use KPI) = rolling ~2-week window.** This is a **current-state gauge**, not a trend, so the oldest raw `fact_duty_daily*.csv` must be **retired** each pull (the glob would otherwise blend an ever-growing cumulative average and misrepresent "how busy are we *now*"). The header "Reporting window" badge is computed from whatever dates the duty file contains (`DATA_WINDOW`), so pruning also keeps that badge honest.
  This asymmetry is intentional: **trend facts grow; the current-state gauge rolls.** Do not "fix" duty to accumulate or workload-util to overwrite.
...
2. **LiteLLM config (P4, P5, P6):** for P4 — split gptoss-120b into two named deployments pointing to the cluster-specific URLs (`inference-spd` for B200, direct H100 vLLM endpoint) so `api_base` cleanly separates them; for P5 — enable `org_alias`; for P6 — pull `team` UUID.
