...
- **Tracked (production) models** — the per-model metrics (duty, token output, cost) count only **production *generative* LLMs**: chat/instruct, vision-language (multimodal), and code models (including the in-house confidential ones). The dashboard answers *"how well are the cards serving real generative inference?"*, so anything that would distort that signal is deliberately excluded: **embeddings & rerankers** (produce vectors/scores, ~0 output tokens → flat-zero noise), **OCR / document utilities**, **test / benchmark / CI** traffic (synthetic load that would *inflate* utilization and token counts), and **`unknown` / MCP** tool-call artifacts (not a served model). Excluded models are **not hidden** — they still occupy cards on the **card map**, folded into the grey **`other`** block (names shown on hover); they're just not given their own duty/token/cost line. The include-list is the `dim_models` whitelist — see `schema.md` and the migration tracker for the mechanism.
...
> **Real-data migration — the real dashboard now renders.** `index.html?data=real` shows the real fleet (maintainer-local; reads gitignored `data_real/`), while the default `index.html` stays the simulated mock. Full tracker + checklist: [`project-log/REAL_DATA_MIGRATION.md`](project-log/REAL_DATA_MIGRATION.md) — **§O = to-do, §T = patch registry** (the short-term workarounds + their drop triggers for production). **Real now:** 236-card fleet (216 B200 + 20 H100, ~54% loaded), duty from **LiteLLM `litellm_requests_metric`**, real Org→App→Model tokens. Trend visuals are **adaptive** (the real window, now ~2 weeks Jun 15–26 — no fabricated history). **Viz 2 = active GPU compute** (DCGM `GPU_UTIL`, card-weighted across both clusters) **24×7 incl. weekends** — not "loaded": models stay resident 24/7, so loaded is a flat ~54%; the weekday ~5–7% / weekend ~0% *active* utilisation is the Batch-Inference idle case (decision + `count()` diagnostic: tracker §N-4). Training shows **"not captured yet · Slurm exporter pending"** (no fabricated data); cost rates placeholder $0.20 for most models (3 have real rates: glm5-1/gptoss/phoenix-vl); `org_alias` not configured → manual team→org map. **Bridge caption** correct on the real view (data-driven; reworded 2026-06-29 so it reads correctly regardless of whether loaded% > or < duty%).
