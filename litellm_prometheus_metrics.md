...
Per-token-type counters that break out the `usage.prompt_tokens_details` and `usage.completion_tokens_details` fields providers report (e.g. OpenAI prompt caching, Anthropic prompt caching, audio I/O, reasoning tokens). These are **additive** to the totals above â the existing `litellm_input_tokens_metric` / `litellm_output_tokens_metric` / `litellm_total_tokens_metric` counters are unchanged.
...
Example PromQL â cache-hit ratio for a model group:
...
Example PromQL â reasoning-token share of output:
...
```
high in_flight_requests + high ALB TargetResponseTime â pod overloaded, scale out
low  in_flight_requests + high ALB TargetResponseTime â delay is pre-ASGI (event loop blocking)
```
...
| `litellm_proxy_total_requests_metric`             | Total number of requests made to the proxy server - track number of client side requests. Labels: `"end_user", "hashed_api_key", "api_key_alias", "requested_model", "team", "team_alias", "user", "status_code", "user_email", "route", "model_id"`. Optionally includes `"stream"` â see [Emit Stream Label](#emit-stream-label).          |
...
## ð¥ LiteLLM Maintained Grafana Dashboards 
